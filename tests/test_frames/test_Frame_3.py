...
class Test(unittest.TestCase):
    def test_0(self):
        ...
        # make sure changing _buffer2d will update frame0.coords too
        assert frame0._buffer2d[19, 0] == frame0[19, 0]
        ...
        #print(frame0._buffer2d[:, 2])
        #print(array('d', frame0._buffer2d[2:4, 0]))
        #print(array('d', frame0._buffer2d[2:4, 1]))
        # #print CA_2
        assert frame0.buffer1d.shape == (912, )
        assert frame0._buffer2d.shape == (304, 3)
        assert frame0.buffer1d.is_c_contig() == True
        #print(array('d', frame0._buffer2d[0, :]))
        frame0._buffer2d[1:3, 0] = array('d', [1., 2.])
        #assert frame0[1:3, 0] == array('d', frame0._buffer2d[1:3, 0])
        #print(array('d', frame0._buffer2d[1:3, 0]))
        #print(frame0[1:3, 0])
        assert_almost_equal(
            frame0[1:3, 0], array('d', frame0._buffer2d[1:3, 0]))
        assert_almost_equal(
            frame0._buffer2d[1:3, 0], array('d', frame0._buffer2d[1:3, 0]))

    def test_1(self):
        traj = mdio.iterload("./data/md1_prod.Tc5b.x", "./data/Tc5b.top")
        frame0 = traj[0]
        #print(frame0[1:5, 2])
        #print(np.asarray(frame0._buffer2d[1:5, :]))
        frame0[1:5, 2] = list(range(100, 104))
        frame0[0, :] = list(range(3))
        #print(frame0._buffer2d[1:5, :])
