...
from pytraj import average_structure
...
class TestAverageFrame(unittest.TestCase):
    ...
    def test_comprehensive(self):
        ...
        # shorter
        frame2 = average_structure(traj)
        aa_eq(frame2.xyz, f_saved.xyz, decimal=3)

        frame3 = average_structure(traj=traj)
        aa_eq(frame3.xyz, f_saved.xyz, decimal=3)

        # test list
        frame4 = average_structure(traj=[traj, traj[:3]], top=traj.top)

        # test iter
        frame5 = average_structure(traj=traj(1, 8, 2), top=traj.top)
        f5_saved = pt.iterload("./data/avg.Tc5b.frame_2_to_8_skip_2.pdb",
                               traj.top)[0]
        aa_eq(frame5.xyz, f5_saved.xyz, decimal=3)

        # test iter CA
        frame5 = average_structure(traj[[0, 3, 7]], '@CA', top=traj.top)

        # use atom_indices
        ca_indices = pt.select('@CA', traj.top)
        frame5_1 = average_structure(traj[[0, 3, 7]], ca_indices, top=traj.top)

        # test frame_indices
        frame6 = average_structure(traj, mask='@CA', frame_indices=[0, 3, 7])
        ...
        # test as traj
        out_traj = average_structure(traj,
                                  mask='@CA',
                                  frame_indices=[0, 3, 7],
                                  dtype='traj')
        ...
        self.assertRaises(ValueError, lambda: pt.average_structure(traj, dtype='trajxyz'))
    ...
    def test_autoimage(self):
        ...
        t0.autoimage()
        avg_0 = pt.average_structure(t0, '@CA')
        avg_1 = pt.average_structure(traj(autoimage=True), '@CA')
    ...
    def test_autoimage_with_rmsfit(self):
        ...
        avg_0 = pt.average_structure(t0, '@CA')
        avg_1 = pt.average_structure(traj(autoimage=True, rmsfit=0), '@CA')
        aa_eq(avg_0.xyz, avg_1.xyz)
        ...
        avg_0 = pt.average_structure(t0, '@CA')
        avg_1 = pt.average_structure(traj(autoimage=True, rmsfit=3), '@CA')
        avg_2 = pt.average_structure(traj, autoimage=True, rmsfit=3, mask='@CA')
        aa_eq(avg_0.xyz, avg_1.xyz)
        ...
        avg_1 = pt.average_structure(traj,
                                  autoimage=True,
                                  rmsfit=5,
                                  mask='@CA',
                                  frame_indices=frame_indices)
        # try with pytraj.Trajectory
        avg_2 = pt.average_structure(t1, autoimage=True, rmsfit=-1, mask='@CA')
        avg_3 = pt.average_structure(traj[:],
                                  autoimage=True,
                                  rmsfit=5,
                                  mask='@CA',
                                  frame_indices=frame_indices)
