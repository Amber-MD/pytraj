import pytraj as pt
from pytraj.testing import cpptraj_dir, aa_eq


def test_xtalsymm(tmpdir):
    with tmpdir.as_cwd():
        x6dky_parm = os.path.join(cpptraj_dir, 'x6dky.parm7')
        x6dky_tname = os.path.join(cpptraj_dir, 'mdXtal.nc')
        x6dky_refname = os.path.join(cpptraj_dir, 'mdXtal.inpcrd')
