...
class PmapDataset(object):
    ...
    def process(self):
        ...
        if self.func in [matrix.dist, matrix.idea, volmap]:
            mat = np.sum((val[0] * val[1] for val in self.data)) / self.traj.n_frames
            return mat
        elif self.func in [ired_vector_and_matrix,]:
            # val : Tuple[(vecs, mat), n_frames]
            mat = np.sum((val[0][1] * val[1] for val in self.data)) / self.traj.n_frames
            vecs = np.column_stack(val[0][0] for val in self.data)
            return (vecs, mat)
        elif self.func in [rotation_matrix, ]:
            ...
            xyz = np.sum((x[1] * x[0].xyz for x in self.data)) / self.traj.n_frames
