...
def calc_distance(traj=None,
    ...
    if 'int' in cm_arr.dtype.name:
        ...
        for cm in list_of_commands:
            if not image:
                cm = ' '.join((cm, _noimage))
            actlist.add_action(c_actions.Action_Distance(),
                               cm,
                               _top,
                               dslist=dslist)
...
def calc_angle(traj=None,
    ...
    if 'int' not in cm_arr.dtype.name:
        if isinstance(command, string_types):
            ...
            for cm in list_of_commands:
                actlist.add_action(c_actions.Action_Angle(),
                                   cm,
                                   _top,
                                   dslist=dslist,
                                   *args,
...
def calc_dihedral(traj=None,
    ...
    if 'int' not in cm_arr.dtype.name:
        if isinstance(command, string_types):
            ...
            for cm in list_of_commands:
                actlist.add_action(c_actions.Action_Dihedral(),
                                   cm,
                                   _top,
                                   dslist=dslist,
                                   *args,
...
def calc_mindist(traj=None,
    ...
    traj = get_fiterator(traj, frame_indices)
    act = c_actions.Action_NativeContacts()
    dslist = CpptrajDatasetList()
...
def calc_diffusion(traj,
    ...
    '''
    act = c_actions.Action_Diffusion()
    dslist = CpptrajDatasetList()
...
def calc_watershell(traj=None,
    ...
    act = c_actions.Action_Watershell()
...
def calc_multivector(traj,
    ...
    >>> vecs = pt.calc_multivector(traj, resrange='1-5', names='C N')
    '''
    act = c_actions.Action_MultiVector()
...
def calc_jcoupling(traj=None,
    ...
    *args, **kwd: optional
    ...
    act = c_actions.Action_Jcoupling()
    dslist = CpptrajDatasetList()
...
def translate(traj=None, command="", frame_indices=None, top=None):
    ...
    c_actions.Action_Translate()(command, fi, top=_top)
...
def do_scaling(traj=None, command="", frame_indices=None, top=None):
    ...
    c_actions.Action_Scale()(command, fi, top=_top)
...
def do_rotation(traj=None, command="", frame_indices=None, top=None):
    ...
    c_actions.Action_Rotate()(command, fi, top=_top)
...
def do_autoimage(traj, command="", frame_indices=None, top=None):
    ...
    '''
    _assert_mutable(traj)
    c_actions.Action_AutoImage()(command, traj, top=top)
    return traj
...
def randomize_ions(traj, mask, around, by, overlap, seed=1, top=None, frame_indices=None):
    ...
    **kwd: other args
    ...
    act = c_actions.Action_RandomizeIons()
...
def calc_multidihedral(traj=None,
    ...
    *arg and **kwd: additional arguments (for advanced users)
    ...
    dslist = CpptrajDatasetList()
    act = c_actions.Action_MultiDihedral()
...
def calc_atomicfluct(traj=None,
    ...
    '''
    dslist = CpptrajDatasetList()
    act = c_actions.Action_AtomicFluct()
    act(mask, traj, top=top, dslist=dslist)
    act.post_process()
    return get_data_from_dtype(dslist, dtype=dtype)
...
def calc_rmsd(traj=None,
    ...
    for cm in command:
        ...
        alist.add_action(c_actions.Action_Rmsd(),
                         _cm,
                         top=_top,
                         dslist=dslist)
...
def check_structure(traj, mask='', frame_indices=None, top=None):
    ...
    """
    act = c_actions.Action_CheckStructure()
...
def rotate_dihedral(traj=None, mask="", top=None):
    ...
    act = c_actions.Action_MakeStructure()
...
def replicate_cell(traj=None, mask="", direction='all', top=None):
    ...
    act = c_actions.Action_ReplicateCell()
    dslist = CpptrajDatasetList()
    act(command, traj, top=_top, dslist=dslist)
...
def _projection(traj,
    ...
    act = c_actions.Action_Projection()
    dslist = CpptrajDatasetList()
...
def calc_atomiccorr(traj,
    ...
    act = c_actions.Action_AtomicCorr()
    dslist = CpptrajDatasetList()
